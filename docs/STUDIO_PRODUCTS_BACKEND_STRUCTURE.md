# STUDIO_PRODUCTS_BACKEND_STRUCTURE.md — Creator Studio: Seller Product-Listing API

> This document describes the **seller product-listing** contract the Creator Studio depends on,
> and how it is wired on the Express backend (`/Users/vinitchuri/code/backend/qatoto-backend`).
> It is scoped to the seller-facing **Create Store Listing** wizard at
> `/studio/products/create` and the **My Products** list at `/studio/products`.
>
> **Read alongside:**
>
> - [STUDIO_PRODUCTS_STRUCTURE.md](STUDIO_PRODUCTS_STRUCTURE.md) — the frontend surface this API
>   feeds (wizard, list, hooks, remaining UI gaps).
> - [STUDIO_BACKEND_STRUCTURE.md](STUDIO_BACKEND_STRUCTURE.md) — video upload (sibling studio
...
> this feature owns the **`/products/*`** routes and the **`product` / `product_image` /
> `product_pricing_tier`** tables.
>
> **Status:** **shipped.** The router is mounted at `/products` in `src/app.ts`; the three
> commerce tables live in `src/db/schema.ts`; the frontend seller surface
> ([STUDIO_PRODUCTS_STRUCTURE.md](STUDIO_PRODUCTS_STRUCTURE.md)) calls these routes through
> `src/lib/products/api.ts`. What remains is mostly **buyer-side catalog** (future store doc) and
> a handful of **frontend polish gaps** (§11).
...
- **My Products** — the seller's listing table
  ...
  wizard; **Edit** opens the same wizard in edit mode via `?id=<productId>`.

B2B **tiered pricing** is part of the commerce model. The backend stores
`product_pricing_tier` rows; the wizard's "Pricing & Inventory" step collects optional bulk tiers
and sends them on create/update (see [STUDIO_PRODUCTS_STRUCTURE.md](STUDIO_PRODUCTS_STRUCTURE.md)
§5).
...
B2B volume pricing — optional tier ladder on the wizard's pricing step. Supported on create and
update; sending `pricingTiers` on PATCH **replaces** the entire set.
...
## 11. Frontend gaps (backend already supports these)

These are **UI follow-ups** on the studio seller surface — the routes exist; the client does not
call or expose them yet. See [STUDIO_PRODUCTS_STRUCTURE.md](STUDIO_PRODUCTS_STRUCTURE.md) §8–§9.

- **List pagination.** `GET /products/mine` is paginated; My Products hardcodes `page=1`.
- **List thumbnails.** `ProductListRow` is compact (no image url); the list shows a generic icon.
  Either extend the list projection or fetch the main image per row.
- **Unpublish from the list.** `POST /products/:id/unpublish` exists; no hook or row action yet.
- **Image reorder.** `PATCH /products/:id/images/reorder` exists; the wizard uploads in pick
  order but has no drag-to-reorder UI for existing images.
- **Video attach picker.** [store-products-picker.tsx](src/components/studio/upload/store-products-picker.tsx)
  still uses hardcoded mock ids; wiring it to `GET /products/mine` (or a compact search endpoint)
  is required so `attachedProductIds` on video create/update pass ownership checks.
...
- **Public buyer catalog.** Active listings visible on `/store` need their own read routes — not
  part of `/products/mine` — specced in the future store backend doc.
