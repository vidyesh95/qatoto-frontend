...
| ~~`lib/videos/studio-view.ts`~~ **CLOSED** | ~~`attachedPitchTitle`, `attachedDocumentNames`~~ | Both are gone. `attachedPitchTitle` became the real `researchProjectSlug` wire field; `attachedDocumentNames` became `savedDocuments`, backed by `POST`/`DELETE /videos/:videoId/documents` and a public `GET …/file`. The claim in this row — "nothing writes it, there is no upload route" — was true and is not any more. **The honest fix WAS a backend field, and it is what shipped**: three routes, Backblaze object storage, and `documents` on the public watch payload so the studio's "shown as a download under the video" copy is finally true |
