...
> and `MOCK_VIDEOS`.
>
> **Eight placeholders across six files remain deliberately mock, each marked `TRANSPORT: mock`.**
...
surface deliberately breaks it.** Eight UI areas across six files have no backend counterpart and
...
| `home/shared/video-card-menu.tsx`       | six of the kebab's eight rows                 | Queue, Download, Not Interested, Don't-recommend and Report are the absences above. **Save to playlist is the interesting one:** `PUT /videos/:videoId/playlists` exists but is OWNER-ONLY — `findUnownedVideoIds` 422s any video the caller did not upload — so it would work on your own cards and fail on every other one. Bookmark and Share ARE wired |
