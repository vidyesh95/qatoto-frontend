// TRANSPORT: client-query — React Query hooks over `@/lib/blueprints/authoring.api`, which is
// itself mock. The hooks are written as if the api were real, because on the day it is, this file
// does not change.
"use client";
 ...
 * accepted, not decided — and there is no queue behind the mock to poll to a verdict. Copying the
 * R&D polling pattern here would re-read the same fixture forever while implying somebody is
 * reviewing.
 *
 ...
 *
 * `onSuccess` invalidates the author's list so that, once a real backend joins the two, the new
 * submission appears without a reload. Against the mock it re-reads six unchanged fixture rows,
 * which is a no-op rather than a lie.
 */
export function useSubmitTeardownMutation() {
  ...
  return useMutation({
    ...
    }) => unwrap(await submitTeardownForReview(variables.draft, variables.idempotencyKey)),
    onSuccess: () => {
