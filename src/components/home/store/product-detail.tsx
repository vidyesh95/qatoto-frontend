...
import { ProductQuantityProvider } from "@/components/home/store/sections/product-quantity-context";
...
import {
  ...
  MOCK_PRODUCT_ID,
  MOCK_PRODUCT_MINIMUM_ORDER_QUANTITY,
...
// The colour the picker below renders as selected, and what the buy actions write against.
//
// IT IS A MODULE CONSTANT BECAUSE THE PICKER IS NOT INTERACTIVE. `selected` is baked into the
// fixture, this is a server component, and nothing lifts a selection into client state — so a buy
// action that claimed to follow the buyer's choice would be describing a control that does not exist.
// Making the picker real is a PDP-wiring change, not a cart one. `prd_folding_chair` declares
// `requiresVariant: true`, so a null here would come back `VARIANT_REQUIRED`.
const SELECTED_PRODUCT_VARIANT_ID =
  MOCK_PRODUCT_COLORS.find((color) => color.selected === true)?.variantId ?? null;
...
export default function ProductDetail({ slug }: { slug: string }) {
  ...
  return (
    // The provider wraps the WHOLE page, not just the buy column: the price chart's stepper sits in
    // that column but the mobile buy bar is a fixed element at the page root, and both need the same
    // quantity. It is a client component with server-rendered children — everything inside stays a
    // server component.
    <ProductQuantityProvider minimumOrderQuantity={MOCK_PRODUCT_MINIMUM_ORDER_QUANTITY}>
      <div className="mx-auto w-full max-w-md pb-40 md:max-w-2xl md:pb-24 lg:max-w-6xl lg:pb-12">
          ...
          color picker first — order utilities only bite at lg. */}
            <div className="lg:flex lg:flex-col">
              ...
              <div className="px-4 pt-2 lg:order-2 lg:px-6">
                <p className="py-2 text-xs font-medium tracking-wide text-foreground">
                  Select Color
                </p>
            ...
            <div className="hidden px-6 py-3 lg:block">
              <BuyActionButtons
                ...
                variantId={SELECTED_PRODUCT_VARIANT_ID}
              />
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-x-0 bottom-[calc(80px+env(safe-area-inset-bottom))] z-20 mx-auto max-w-md bg-white px-4 py-2 md:bottom-0 lg:hidden">
          <BuyActionButtons productId={MOCK_PRODUCT_ID} variantId={SELECTED_PRODUCT_VARIANT_ID} />
        </div>
      </div>
