...
// TRANSPORT: mock — the product comment thread. NOTHING here reaches a backend.
//
// This is the OLD watch-page comments component, moved rather than deleted when the video
// thread was wired to `GET /videos/:id/comments`. The store surface has no equivalent route:
// there is no product-comment table, no product-review table, and nothing joins a review to an
// order line to make "Verified Purchase" mean anything. Coupling it to the video thread would
// have made a store product's comments show a video's comments, which is worse than a mock.
//
// It renders `Comment[]` / `Review[]` from `@/types/video`, which the caller supplies as local
// consts. Deliberate; see docs/HOME_STRUCTURE.md §10.
//
// The Top/New sort pills below sort NOTHING and never did. They are kept only because the mock
// is a layout study; do not copy this pattern to a wired surface.
...
export default function ProductCommentThread({
  count,
  comments,
  trending,
  saleItem,
