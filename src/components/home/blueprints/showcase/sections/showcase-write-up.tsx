...
// TRANSPORT: props-only — the write-up arrives on the showcase from `getBlueprintByCategory`.
//
...
// the same way `BrowserPreferencesProvider` does not make `app/layout.tsx`'s children client.
//
...
/**
 * Blank-line separated, the way somebody types into a textarea.
 *
 ...
 * page. An entry that is nothing but whitespace yields no paragraphs at all, which the caller
 * renders as the absence it is.
 */
function splitIntoParagraphs(writeUp: string): readonly string[] {
  return writeUp
    .split(/\n\s*\n/)
...
export default function ShowcaseWriteUp({ writeUp }: { readonly writeUp: string }) {
  ...
  useEffect(() => {
    ...
    // take "Show less" off the screen with it, leaving a reader expanded with no way back.
    ...
    // `writeUp` IS DELIBERATELY NOT A DEPENDENCY, and the caller carries the other half of that:
    // it keys this component by slug, so a different launch is a different instance rather than
    // the same one holding a stale measurement. The observer cannot cover it — the crop's border
    // box stays at 9rem whatever the text does inside it, so new text fires no resize.
  }, [isExpanded]);

  // A write-up of nothing but whitespace is `null` wearing a string, and absence renders nothing.
  ...
  return (
    <div className="mt-4">
      ...
      {/* THE CONTROL RENDERS ONLY WHEN IT HAS SOMETHING TO DO. A write-up under the crop shows no
